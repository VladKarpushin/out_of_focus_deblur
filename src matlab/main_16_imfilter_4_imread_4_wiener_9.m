...
%strFileName = strcat(strFolder,'DSC_0138_cut.JPG');
...
%************
blurred_noisy = imgA;
NSR = 0.001;      % NSR is the noise-to-signal power ratio of the additive noise
blurred_noisy = edgetaper(blurred_noisy,fspecial('gaussian',60,3));      %The output image J could exhibit ringing introduced by the discrete Fourier transform used in the algorithm

%blurred_noisy = padarray(blurred_noisy,[50 50]);

PSF = MyCircle(N, R);   % point-spread function with which I was convolved
wnr3    =   deconvwnr(blurred_noisy, PSF, NSR);
wnrMy   = MyDeconvwnr(blurred_noisy, PSF, NSR);
wnrMy = ifftshift(wnrMy);
